...
def start_redis(port):
  redis_filepath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "redis-server")
...
def start_local_scheduler(redis_address, plasma_store_name):
  local_scheduler_filepath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "photon_scheduler")
...
def start_ray_local(node_ip_address="127.0.0.1", num_objstores=1, num_workers=0, worker_path=None):
  ...
  time.sleep(0.2)
  for i in range(num_objstores):
    object_store_name, object_store_manager_port = object_store_info[i]
